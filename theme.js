...
// Generates auto-fit and auto-fill grids based on a minimum column width suffix (in pixels)
let autoColumns = [];

for (let i = 1; i < 51; i++) {
  autoColumns.push([
    `auto-fit-${i * 10}`,
    `repeat(auto-fit, minmax(min(100%, ${(i * 10) / 16}rem), 1fr))`
  ]);

  autoColumns.push([
    `auto-fill-${i * 10}`,
    `repeat(auto-fill, minmax(min(100%, ${(i * 10) / 16}rem), 1fr))`
  ]);
}
...
export const theme = {
  ...
  extend: {
    ...
    gridTemplateColumns: { ...Object.fromEntries(columns) },
    opacity: {
      '15': '.15',
      '35': '.35',
      '45': '.45',
